...
class CurrencyConverter extends AbstractMathematicalConverter
{
    ...
    protected function normalize(ConvertibleValue $cv)
    {
        switch ($cv->getUnit()) {
            ...
            case self::$jpy:
                $cv->setValue($cv->getValue() / self::$jpy->getFactor());
                break;

            case self::$bgn:
                $cv->setValue($cv->getValue() / self::$bgn->getFactor());
                break;

            case self::$czk:
                $cv->setValue($cv->getValue() / self::$czk->getFactor());
                break;

            case self::$dkk:
                $cv->setValue($cv->getValue() / self::$dkk->getFactor());
                break;

            case self::$gbp:
                $cv->setValue($cv->getValue() / self::$gbp->getFactor());
                break;

            case self::$huf:
                $cv->setValue($cv->getValue() / self::$huf->getFactor());
                break;

            case self::$pln:
                $cv->setValue($cv->getValue() / self::$pln->getFactor());
                break;

            case self::$ron:
                $cv->setValue($cv->getValue() / self::$ron->getFactor());
                break;

            case self::$sek:
                $cv->setValue($cv->getValue() / self::$sek->getFactor());
                break;

            case self::$chf:
                $cv->setValue($cv->getValue() / self::$chf->getFactor());
                break;

            case self::$nok:
                $cv->setValue($cv->getValue() / self::$nok->getFactor());
                break;

            case self::$hrk:
                $cv->setValue($cv->getValue() / self::$hrk->getFactor());
                break;

            case self::$rub:
                $cv->setValue($cv->getValue() / self::$rub->getFactor());
                break;

            case self::$try:
                $cv->setValue($cv->getValue() / self::$try->getFactor());
                break;

            case self::$aud:
                $cv->setValue($cv->getValue() / self::$aud->getFactor());
                break;

            case self::$brl:
                $cv->setValue($cv->getValue() / self::$brl->getFactor());
                break;

            case self::$cad:
                $cv->setValue($cv->getValue() / self::$cad->getFactor());
                break;

            case self::$cny:
                $cv->setValue($cv->getValue() / self::$cny->getFactor());
                break;

            case self::$hkd:
                $cv->setValue($cv->getValue() / self::$hkd->getFactor());
                break;

            case self::$idr:
                $cv->setValue($cv->getValue() / self::$idr->getFactor());
                break;

            case self::$ils:
                $cv->setValue($cv->getValue() / self::$ils->getFactor());
                break;

            case self::$inr:
                $cv->setValue($cv->getValue() / self::$inr->getFactor());
                break;

            case self::$krw:
                $cv->setValue($cv->getValue() / self::$krw->getFactor());
                break;

            case self::$mxn:
                $cv->setValue($cv->getValue() / self::$mxn->getFactor());
                break;

            case self::$myr:
                $cv->setValue($cv->getValue() / self::$myr->getFactor());
                break;

            case self::$nzd:
                $cv->setValue($cv->getValue() / self::$nzd->getFactor());
                break;

            case self::$php:
                $cv->setValue($cv->getValue() / self::$php->getFactor());
                break;

            case self::$sgd:
                $cv->setValue($cv->getValue() / self::$sgd->getFactor());
                break;

            case self::$thb:
                $cv->setValue($cv->getValue() / self::$thb->getFactor());
                break;

            case self::$zar:
                $cv->setValue($cv->getValue() / self::$zar->getFactor());
                break;
        }
